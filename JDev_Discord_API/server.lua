...
Citizen.CreateThread(function()
	if (GetCurrentResourceName() ~= "JDev_Discord_API") then 
		--StopResource(GetCurrentResourceName());
		print("[" .. GetCurrentResourceName() .. "] " .. "IMPORTANT: This resource must be named JDev_Discord_API for it to work properly with other scripts...");
	end
end)

function sendDebugMessage(msg)
	print("^1[^5JDev_Discord_API^1] ^3" .. msg);
end

tracked = {}

RegisterNetEvent('JDev_Discord_API:PlayerLoaded')
AddEventHandler('JDev_Discord_API:PlayerLoaded', function()
	if (GetCurrentResourceName() ~= "JDev_Discord_API") then 
		TriggerClientEvent('chatMessage', -1, '^1[^5SCRIPT ERROR^1] ^3The script ^1' .. GetCurrentResourceName() .. ' ^3will not work properly... You must '
	.. 'rename the resource to ^1JDev_Discord_API');
	end
	...
	if (tracked[license] == nil) then 
		...
		'^1[^5JDev_Discord_API^1] ^3The Discord API script was created by JDev. You may join his Discord at: ^6discord.gg/')
	end
	TriggerClientEvent('chatMessage', source, 
		'^7[^2Zap-Hosting^7] ^3Use code ^5TheWolfJDev-4765 ^3at checkout for ^220% ^3off of selected services. Visit ^5https://zap-hosting.com/JDev ^3to get started!');
end)

card = '{"type":"AdaptiveCard","$schema":"http://adaptivecards.io/schemas/adaptive-card.json","version":"1.4","body":[{"type":"Image","url":"' .. Config.Splash.Header_IMG .. '","horizontalAlignment":"Center"},{"type":"Container","items":[{"type":"TextBlock","text":"JDev_Discord_API","wrap":true,"fontType":"Default","size":"ExtraLarge","weight":"Bolder","color":"Light","horizontalAlignment":"Center"},{"type":"TextBlock","text":"' .. Config.Splash.Heading1 .. '","wrap":true,"size":"Large","weight":"Bolder","color":"Light","horizontalAlignment":"Center"},{"type":"TextBlock","text":"' .. Config.Splash.Heading2 .. '","wrap":true,"color":"Light","size":"Medium","horizontalAlignment":"Center"},{"type":"ColumnSet","height":"stretch","minHeight":"100px","bleed":true,"horizontalAlignment":"Center","columns":[{"type":"Column","width":"stretch","items":[{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Discord","url":"' .. Config.Splash.Discord_Link .. '","style":"positive"}],"horizontalAlignment":"Right"}],"height":"stretch"},{"type":"Column","width":"stretch","items":[{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Website","style":"positive","url":"' .. Config.Splash.Website_Link .. '"}],"horizontalAlignment":"Left"}]}]},{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"cliquer ici pour rejoindre Juliendev Discord","style":"destructive","iconUrl":"","url":"https://discord.com/invite/"}],"horizontalAlignment":"Center"}],"style":"default","bleed":true,"height":"stretch"},{"type":"Image","url":"","selectAction":{"type":"Action.OpenUrl","url":""},"horizontalAlignment":"Center"}]}'
...
function IsDiscordEmailVerified(user) 
    ...
    if discordId then 
        ...
        if member.code == 200 then
        	...
        	sendDebugMessage("[JDev_Discord_API] ERROR: Code 200 was not reached. DETAILS: " .. error_codes_defined[member.code]);
        end
    end
    return isVerified;
end
...
function GetGuildIcon(guild --[[optional]])
  local guildId = GetGuildId(guild)
	...
	if guild.code == 200 then
		...
		sendDebugMessage("[JDev_Discord_API] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data or guild.code)) 
	end
	return nil;
end
...
function GetDiscordAvatar(user) 
	...
	if discordId then 
		...
		sendDebugMessage("[JDev_Discord_API] ERROR: Discord ID was not found...")
	end
    return imgURL;
end
...
function GetDiscordRoles(user, guild --[[optional]])
  ...
  local roles = nil
	...
	if discordId then
		...
		sendDebugMessage("[JDev_Discord_API] ERROR: Discord was not connected to user's Fivem account...")
		return false
	end
	return false
end
...
function GetUserRolesInGuild (user, guild)
	...
	if not user then
		sendDebugMessage("[JDev_Discord_API] ERROR: GetUserRolesInGuild requires discord ID")
		return false
	end
	if not guild then
		sendDebugMessage("[JDev_Discord_API] ERROR: GetUserRolesInGuild requires guild ID")
		return false
	end
	...
	if member.code == 200 then
		...
		sendDebugMessage("[JDev_Discord_API] ERROR: Code 200 was not reached... Returning false. [Member Data NOT FOUND] DETAILS: " .. error_codes_defined[member.code])
		return false
	end
end
  ...
  local guildId = GetGuildId(guild)
	...
	if discordId then
		...
		if member.code == 200 then
			...
			sendDebugMessage("[JDev_Discord_API] ERROR: Code 200 was not reached. Error Code: " .. error_codes_defined[member.code])
			return nil;
		end
	else
		sendDebugMessage("[JDev_Discord_API] ERROR: Discord was not connected to user's Fivem account...")
...
Citizen.CreateThread(function()
  ...
  if mguild.code == 200 then
    ...
    sendDebugMessage("[JDev_Discord_API] Successful connection to Guild : "..data.name.." ("..data.id..")")
  else
    sendDebugMessage("[JDev_Discord_API] An error occured, please check your config and ensure everything is correct. Error: "..(mguild.data and json.decode(mguild.data) or mguild.code)) 
  end
  if (Config.Multiguild) then 
    for _,guildID in pairs(Config.Guilds) do
      ...
      if guild.code == 200 then
        ...
        sendDebugMessage("[JDev_Discord_API] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data and json.decode(guild.data) or guild.code)) 
      end
    end
  end
end)
